...
def test_new_level():
    pdssp.custom_logging.UtilsLogs.add_logging_level("TRACE", 20)
...
def test_message():
    record = pdssp.custom_logging.LogRecord(
        "__main__",
        logging.INFO,
        "pathname",
        2,
...
def test_color_formatter():
    formatter = pdssp.custom_logging.CustomColorFormatter()
    logger = logging.getLogger("pdssp.custom_logging")
    ...
    shell_formatter = pdssp.custom_logging.ShellColorFormatter()
    record = pdssp.custom_logging.LogRecord(
        "__main__",
        logging.INFO,
        "pathname",
