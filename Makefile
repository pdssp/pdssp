...
Usage:\n
	...
	\t\tInstallation\n
	-------------------------------------------------------------------------\n
	make\t\t\t\t                Install pdssp in the system (root)\n
	make user\t\t\t 			Install pdssp for non-root usage\n
	\n
	-------------------------------------------------------------------------\n
	\t\tDevelopment\n
	-------------------------------------------------------------------------\n
	make prepare-dev\t\t 		Prepare Development environment\n
	make install-dev\t\t 		Install COTS and pdssp for development purpose\n
...
prepare-dev:
	cp .pypirc ~${USER} && git init && echo "python3 -m venv pdssp-env && export PYTHONPATH=. && export PATH=`pwd`/pdssp-env/bin:${PATH}" > ${VENV} && echo "source \"`pwd`/pdssp-env/bin/activate\"" >> ${VENV} && scripts/install-hooks.bash && echo "\nnow source this file: \033[31msource ${VENV}\033[0m"
...
lint:  ## Lint and static-check
	flake8 --ignore=E203,E266,E501,W503,F403,F401 --max-line-length=79 --max-complexity=18 --select=B,C,E,F,W,T4,B9 pdssp
	pylint pdssp
	mypy pdssp
...
visu-doc-pdf:
	acroread docs/build/latex/pdssp.pdf
